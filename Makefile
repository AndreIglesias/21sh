...
#    Updated: 2021/09/10 22:25:31 by ciglesia         ###   ########.fr        #
#                                                                              #
#******************************************************************************#
...
DIREDT		=	$(DIRTRM)/line_editor/

SRC			=	main.c
SIGNAL		=	signals.c
TERM		=	terminal.c environment.c getcmd.c keys_events.c prompt.c git_prompt.c
EDITOR		=	multiline.c move_cursor.c calculations.c delete.c select.c clipboard.c
...
ANALIZE		=	analyzer.c lexer.c parser.c semantic.c  boolean.c inception_tree.c extract_tokens.c replace_var.c
...
SRCS		=	$(SRC) $(SIGNAL) $(TERM) $(HISTORY) $(BUILTIN) $(EVAL) $(ANALIZE) $(AST) $(AUTOCP) $(EDITOR)
...
%.o		:		../$(DIRSRC)/%.c
				@printf $(GREEN)"Generating 21sh objects... %-33.33s\r" $@
